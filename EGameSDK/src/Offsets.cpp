...
namespace EGSDK {
	...
	static std::vector<VersionedRegistryRow> BuildVersionedGameRegistry() {
		...
		rows.push_back(MakeNamed("TryGetBaseAIManager", kDllGame, SortPat({
			{ 12001u, { "40 53 48 83 EC ?? 48 83 B9 ?? ?? ?? ?? ?? 48 8B D9 75 ?? 41 B9 ?? ?? ?? ?? 48 8D 05 ?? ?? ?? ?? 48 89 44 24 ?? 4C 8D 05 ?? ?? ?? ?? 48 C7 44 24 ?? ?? ?? ?? ?? 48 8D 15 ?? ?? ?? ?? C7 44 24 ?? ?? ?? ?? ?? 41 8D 49 ?? FF 15 ?? ?? ?? ?? 48 8B 83 ?? ?? ?? ?? 48 83 C4 ?? 5B C3 48 8B 81 ?? ?? ?? ?? 48 83 C4 ?? 5B C3 CC CC CC CC CC CC CC CC 40 53", PT::Address } },
			})));
		rows.push_back(MakeNamed("BindManagerToBaseAI", kDllGame, SortPat({
			{ 12001u, { "48 89 8A ?? ?? ?? ?? C3 CC CC CC CC CC CC CC", PT::Address } },
			})));
		...
		rows.push_back(MakeMember(&LevelDI::pBaseAIManager, SortMem({
			Mem1(12001u, kDllGame, "48 8B 9F [?? ?? ?? ?? FF 90 ?? ?? ?? ?? 0F 28 C8 48 8B CB E8 ?? ?? ?? ?? 48 8B CF E8 ?? ?? ?? ?? 48 8B D7", PT::MemberDisplacement32),
			})));
		rows.push_back(MakeMember(&BaseAI::pBaseAIManager, SortMem({
			Mem1(12001u, kDllGame, "48 89 8A [?? ?? ?? ?? C3 CC CC CC CC CC CC CC", PT::MemberDisplacement32),
			})));
